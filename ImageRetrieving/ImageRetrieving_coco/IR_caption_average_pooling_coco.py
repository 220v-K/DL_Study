...
# -----------------------------------------------------------------------------
class ImageTextModelWithCaptionPooling(pl.LightningModule):
    def __init__(self,
                 ...
                 temperature=0.07,
                 learning_rate=1e-4,
                 should_train_image_encoder=False,
                 should_train_text_encoder=False):
        super().__init__()
        ...
        # 인코더 동결 여부 설정
        if not should_train_image_encoder:
            for param in self.image_encoder.parameters():
                param.requires_grad = False
        
        if not should_train_text_encoder:
    ...
    def compute_contrastive_loss(self, image_embeds, text_embeds):
        ...
        Args:
            ...
            text_embeds: 텍스트 임베딩 (캡션 평균) [batch_size, embed_dim]
        """
    ...
    def configure_optimizers(self):
        ...
        if self.should_train_image_encoder:
            param_groups.append({
                ...
                "lr": self.learning_rate * 0.1  # 인코더는 더 작은 학습률 사용
            })
...
if __name__ == "__main__":
    ...
    model = ImageTextModelWithCaptionPooling(
        image_encoder_name="microsoft/swin-base-patch4-window7-224",
        text_encoder_name="roberta-large",
        embed_dim=256,
        temperature=0.07,
        learning_rate=1e-4,
        should_train_image_encoder=False,  # 이미지 인코더 학습 여부
        should_train_text_encoder=False    # 텍스트 인코더 학습 여부
    )
    
    # 체크포인트 저장 경로
    checkpoint_dir = "./checkpoints/coco_caption_pooling"
    ...
    # 로거 설정
    logger = TensorBoardLogger("./logs", name="coco_caption_pooling")
    ...
    trainer = pl.Trainer(
        ...
        accelerator="auto",
        devices=1,
        precision="16-mixed",  # 메모리 효율성을 위한 혼합 정밀도 사용
