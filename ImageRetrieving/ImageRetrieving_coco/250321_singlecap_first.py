...
# -----------------------------------------------------------------------------
class COCODatasetWithFirstCaption(Dataset):
    """
    MS-COCO 데이터셋을 로드하고 각 이미지에 대해 첫 번째 캡션만 사용하는 클래스
    """
    def __init__(self, split='train', root_dir='./ImageRetrieving_coco', image_size=224, max_length=77):
        ...
        print(f"총 {len(self.image_ids)}개의 유니크한 이미지가 있습니다.")
    ...
    def __getitem__(self, idx):
        """
        하나의 이미지와 해당 이미지에 대한 첫 번째 캡션을 반환
        """
...
# 단일 캡션용 collate 함수
def single_caption_collate_fn(batch):
    batch_dict = {
        "image": [],
        "input_ids": [],
        "attention_mask": [],
    ...
    for item in batch:
        ...
        batch_dict["caption"].append(item["caption"])
...
# 3. DataModule
# -----------------------------------------------------------------------------
class COCODataModuleWithFirstCaption(pl.LightningDataModule):
    def __init__(self,
                batch_size=128,
                num_workers=4,
                image_size=224,
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            # 훈련 데이터셋 로드
            self.train_dataset = COCODatasetWithFirstCaption(
                split='train',
            ...
            # 검증 데이터셋 로드
            self.valid_dataset = COCODatasetWithFirstCaption(
                split='val',
                root_dir=self.root_dir,
                image_size=self.image_size,
                max_length=self.max_length
        ...
        if stage == "test" or stage is None:
            ...
            self.test_dataset = COCODatasetWithFirstCaption(
                split='test',
                root_dir=self.root_dir,
                image_size=self.image_size,
                max_length=self.max_length
    ...
    def train_dataloader(self):
        return DataLoader(
            ...
            pin_memory=True,
            drop_last=True,
            collate_fn=single_caption_collate_fn,
...
class EnhancedImageTextModel(pl.LightningModule):
    ...
    def encode_text(self, input_ids, attention_mask):
        ...
        # CLS 토큰의 임베딩 추출
        text_features = text_outputs.last_hidden_state[:, 0]  # [batch_size, hidden_size]
        
        # 프로젝션 및 정규화
        text_embeds = self.text_proj(text_features)  # [batch_size, embed_dim]
    ...
    def forward(self, batch):
        ...
        # 텍스트 인코딩 (단일 캡션)
        text_embeds = self.encode_text(input_ids, attention_mask)  # [batch_size, embed_dim]
...
if __name__ == "__main__":
    # 데이터 모듈 초기화
    data_module = COCODataModuleWithFirstCaption(
        batch_size=64,
        num_workers=4,
        image_size=224,
        max_length=77,
    ...
    # 체크포인트 저장 경로
    checkpoint_dir = "./checkpoints/coco_firstcap_hard_mining"
    ...
    # 로거 설정
    logger = TensorBoardLogger("./logs", name="coco_firstcap_hard_mining")
    ...
    trainer = pl.Trainer(
        ...
        devices=1,
        precision="16-mixed",
        # strategy="deepspeed_stage_2",  # DeepSpeed Zero-2 전략 적용
